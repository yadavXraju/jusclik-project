...
const ConfigureStudentWise = () => {
  ...
  const handleDeleteRow = (id) => {
    const row = configStudentWiseData.find((row) => row.id === id);
    setRowToDelete(row);
    setModalOpen(true);
    setEditingRow(null);
  };
  ...
  const handleSaveRow = () => {
    if (editingRow) {
      const updatedData = configStudentWiseData.map((row) =>
      ...
      <Grid item xs={12} lg={12} sx={{ marginTop: '3rem' }}>
        <TableContainer component={Paper} sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
          <Table sx={{ width: '100%' }} aria-label="simple table">
            ...
            <TableBody className="scrollbar" sx={{ display: 'flex', maxHeight: '300px', flexDirection: 'column', overflowY: 'auto', paddingBottom: '60px', minHeight: '120px' }}>
              {data.map((rowData) => (
