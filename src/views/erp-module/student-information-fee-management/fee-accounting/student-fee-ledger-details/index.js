...
export default function StudentFeeLedgerDetails() {
  ...
  // Calculate total net payable amount received
  const totalNetPayAmt = rows.reduce((acc, row) => acc + row.netpay, 0);
  const amountReceived = rows.reduce((acc , row)=> acc + row.amtpay , 0);
  const balanceFee = rows.reduce((acc , row)=> acc + row.bal , 0);
                    ...
                    {visibleRows.map((row, index) => {
                      ...
                      return (
                        ...
                        <TableCell
                          ...
                          {/* Display total net payable amount below the "Total Net Payable" column header */}

                      <TableRow>
                            <TableCell align="right"  sx={{fontSize:'18px' , fontWeight:'600'}}>Total </TableCell>
                            <TableCell align="right"  sx={{fontWeight:'600'}} colSpan={4}> {totalNetPayAmt}</TableCell>
                            <TableCell align="right" sx={{fontWeight:'600'}} colSpan={1}> {amountReceived}</TableCell>
                            <TableCell align="right" sx={{fontWeight:'600'}} colSpan={1}> {balanceFee}</TableCell>
                      </TableRow>
