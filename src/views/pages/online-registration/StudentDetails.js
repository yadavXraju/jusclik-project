...
function StudentDetails() {
  const [TabChange, setTabChange] = React.useState(2);
...
{ value: 'others', label: 'Others' },
  ...
  return (
    ...
    <Grid sx={{background:'#EEF2F6'}}> 
        ...
        <Grid container spacing = {2}  sx={{paddingLeft:'6rem' , paddingTop:'4rem' , paddingBottom:'6rem' }}>

           <Grid item xs={2}  >
            <Box>
            
            <Typography variant='h3' sx={{ fontWeight: 'bold', display: 'flex', alignItems: 'center',paddingTop:'2rem' }}>
      <img src={CloudLogo} alt='Logo' style={{ marginRight: '8px'  }} /> REGISTRATION FORM  </Typography>
                                                                                            
              </Box>

              <Box sx={{ width: '60%' , paddingTop:'2rem' }}>
            <Tabs value={TabChange} onChange={handleChange} centered orientation='vertical' sx={{ '& .MuiTabs-indicator': { left: 0, right: 'auto' } }}>
             <Tab sx={{fontSize:'14px'}} label="STUDENT'S DETAILS" />
             <Tab label="Item Two" />
           ...
           <Paper>
           <Box sx={{padding:'2rem'}}>
           <Typography variant='h3' sx={{ fontWeight: 'bold', paddingBottom:'1rem' }}> STEP 1 : STUDENT&apos;S DETAILS </Typography>
           <Divider/>
          </Box>
