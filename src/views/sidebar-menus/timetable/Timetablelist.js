...
const rows = [
  ...
  createData('', 'Brack', '11:00am to 11:15am'),
  createData(4, 'Hindi', '11:15am to 12:00am'),
  createData(5, 'Physics', '12:00am to 12:45pm'),
  createData('', 'Lunch Brack', '12:45am to 01:45pm'),
      ...
      {rows.map((row) => (
        <Grid item xs={12} key={row.key}>
          <Paper elevation={3} style={{ padding: '15px', textAlign: 'center', display: 'flex',lineHeight: '10px' }}>
              <Typography variant="h6" sx={{ background:'#ede7f6' }}>
              <Box sx={{ m: 2 }}>{row.key}</Box>
              </Typography>
              <Typography variant="body1">
              <Box sx={{ m: 2 }}>{row.sub}</Box>
              </Typography>
              <Typography variant="body1" sx={{ marginLeft: 'auto' }}>
              <Box sx={{ m: 2 }}>{row.time}</Box>
              </Typography>
          </Paper>
        </Grid>
      ))}
