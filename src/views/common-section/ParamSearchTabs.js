    ...
    <>
      <Box sx={{ flexGrow: 1, display: 'flex', background: "none" }}>
        <Box>
          ...
          >
            {filterData && filterData.map((tab, index) => (
              <Tab key={tab?.id} label={tab.name} {...a11yProps(index)} sx={{ borderBottom: "1px solid #eef2f6", width: ' 100%', alignItems: "baseline" }}  />
            ))}
          </Tabs>
        </Box>
        {
          filterData && filterData.map((tab, index) => {
            return <TabPanel key={tab?.id} value={value} index={index} styles={tabPanelStyles}>
              <tab.component  {...tab.props} />
            </TabPanel>
          })
        }
